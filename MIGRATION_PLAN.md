...
- New env var: `TELEGRAM_CHAT_ID_MONDAY` (added to `.env.example`) - Wednesday's poll reuses the existing `TELEGRAM_CHAT_ID`; Monday's needs its own value, equivalent to backend's `API_TG_GROUPS_MONDAY_GROUPID`. Not populated anywhere yet - needs a real value before Monday's poll can actually send.
