...
**New GitHub secrets to create** (Repo Settings → Environments → `production-dutchlankanshuttlemasters`, or wherever the existing ones live) before running this workflow - one `_V3`-suffixed secret per existing one used by the real workflow:
`NEXT_PUBLIC_BACKEND_URL_V3`, `NEXT_PUBLIC_AUTH_ENABLED_V3`, `NEXTAUTH_URL_V3`, `ALLOWED_ADMIN_EMAILS_V3`, `GOOGLE_CLIENT_ID_V3`, `GOOGLE_CLIENT_SECRET_V3`, `NEXTAUTH_SECRET_V3`, `NEXT_PUBLIC_APP_URL_V3`, `TELEGRAM_BOT_TOKEN_V3`, `TELEGRAM_CHAT_ID_V3`, `SENTRY_AUTH_TOKEN_V3`, `DATABASE_URL_V3`. (`GHCR_PUSH_TOKEN` is reused as-is - it's just registry auth, not app config.)

Guidance on values - most can just be copied from the existing secret if testing in place on the same domain; a few need real thought:
- **`DATABASE_URL_V3` (most important):** must point at a reachable MySQL with the `brs` schema (Phase 5's blocking prerequisite - without this, login is broken). Strongly consider pointing this at a **copy** of `brs`, not the real production database, for initial testing - Phase 4 found that processing encounters has real side effects on every active player who didn't play (correct behavior, but surprising if you're not intending it on real data during a test).
- **`NEXTAUTH_URL_V3` / `NEXT_PUBLIC_APP_URL_V3`:** must match wherever the v3.0 container is actually reachable. If it's genuinely a different URL/subdomain than production, `GOOGLE_CLIENT_ID_V3`/`GOOGLE_CLIENT_SECRET_V3` likely need to be a separate OAuth client with that redirect URI registered in Google Cloud Console - reusing the production OAuth client only works if testing on the exact same origin.
- Everything else (`ALLOWED_ADMIN_EMAILS_V3`, `TELEGRAM_*_V3`, `SENTRY_AUTH_TOKEN_V3`) can typically just mirror the existing values.
...
**Once this is stable:** per the existing Phase 7 versioning note, rename the `_V3` secrets back to their plain names and retire the old `v2.0` workflow/tag - not done now, since nothing has been confirmed working yet.
