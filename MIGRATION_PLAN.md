...
- **Batch B (player writes)** - not started: `pages/api/players/[id].ts` (PUT), `pages/api/players/[id]/activate.ts`, `pages/api/players/index.ts`'s `POST` branch.
- **Batch C (encounter processing, last, own deploy)** - not started: `pages/api/games/[id]/submit.ts`, `pages/api/games/[id]/process.ts`. Highest-stakes pair (real absentee side effect on non-participating active players) - isolated deliberately so a rollback target is unambiguous, and best validated against a real low-stakes game day rather than synthetic data.
