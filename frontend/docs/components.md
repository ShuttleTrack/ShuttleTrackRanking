...
**Layout:** `max-w-7xl` shell; **Check in** title + orange rule. Hero card: session date (`text-xl`), then one line with `font-numeric` time range (full IANA zone on hover), **starts-in / live / ended** chip, and a thin divider into a compact two-column row — **Voting closes** (`at HH:mm` + live countdown at `text-lg`, or **Voting closed**) and **Confirmed** (label + count, right-aligned).

**By role** (from the API): **voter** — **I'm in** / **I'm out** (stacked on mobile, side-by-side `sm+`; orange fill for In, red for Out), each disabled from the server's own rule table with its reason shown (e.g. "Voting has closed"); after the first vote the pair hides behind an outline **Change your vote** control (no expand when switching is impossible, e.g. direct claimer); reopening shows both buttons with the current choice disabled; a direct claimer sees **I'm in** only; an assignee or a holder of a slot passed on after the deadline gets a one-line prompt to confirm. **Open-slot player** — **Join waiting list** / **Leave waiting list** (with their position) before the deadline, **Claim a slot** after, in the same two-button geometry. **Observer** (a covered fulltime owner, a superadmin with no player row) — roster plus one line saying why they cannot act.
