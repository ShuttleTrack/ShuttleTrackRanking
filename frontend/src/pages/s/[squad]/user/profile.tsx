...
const UserProfilePage = ({ playerId }: UserProfilePageProps) => {
  ...
  const scoreDisplay = currentUser?.rankScore != null ? currentUser.rankScore.toFixed(1) : '—';
  const highestDisplay =
    currentUser?.highestRank != null && currentUser.highestRank > 0 ? `#${currentUser.highestRank}` : '—';
      ...
      <section className="max-w-3xl rounded-xl border border-gray-600 bg-surface-container overflow-hidden">
        <div className="flex items-center justify-between gap-3 border-b border-gray-600 p-4">
          <div className="flex min-w-0 items-center gap-3">
            {session?.user?.image ? (
              ...
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border-2 border-primary bg-surface-container-high font-headline font-semibold text-on-surface">
                {avatarInitial}
              </div>
            )}
            <div className="min-w-0">
              <div className="font-headline font-semibold text-on-surface truncate">{displayName}</div>
        ...
        <div className="grid grid-cols-4 divide-x divide-gray-600">
          <div className="p-3 text-center">
            ...
            <div
              className={`font-numeric mt-1 text-lg font-bold tabular-nums ${
                rankDisplay === '—' ? 'text-on-surface-variant' : 'text-primary'
              }`}
            >
              {rankDisplay}
            </div>
          ...
          <div className="p-3 flex flex-col items-center justify-center">
            ...
            <div className="mt-1">
              {currentUser ? (
                ...
                <span className="font-numeric text-lg font-bold tabular-nums text-on-surface-variant">—</span>
              )}
            </div>
          </div>
          <div className="p-3 text-center">
            ...
            <div
              className={`font-numeric mt-1 text-lg font-bold tabular-nums ${
                scoreDisplay === '—' ? 'text-on-surface-variant' : 'text-on-surface'
              }`}
            >
              {scoreDisplay}
            </div>
          ...
          <div className="p-3 text-center">
            ...
            <div
              className={`font-numeric mt-1 text-lg font-bold tabular-nums ${
                highestDisplay === '—' ? 'text-on-surface-variant' : 'text-on-surface'
              }`}
            >
              {highestDisplay}
            </div>
