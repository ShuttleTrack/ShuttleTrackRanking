...
// here with the same plain-text response and a thrown error for the empty case. Guarded to match
// the existing pages/api/games/[id]/process.ts caller (admin auth) - this is the route with the
// real absentee-deduction side effect on every non-participating active player, so this guard
// matters more here than anywhere else in this batch.
