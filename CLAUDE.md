...
- **Game-day check-in** (attendance vote, open-slot waiting list, per-squad Telegram posts): `frontend/src/lib/gameDay/`, with its own every-5-minutes cron registered beside the poll in `instrumentation.ts`. Pure decision logic (`clock.ts`, `voteWindow.ts`, `counts.ts`, `votes.ts`'s `evaluateVote`, `scheduler.ts`'s `decide*`) is kept separate from the Prisma paths; the tests run the Prisma paths against a stateful fake (`lib/gameDay/testing/fakePrisma.ts`).
