...
- **Group size order is random but fixed per day:** `calculateGroupDistribution(total, seed)` fixes the size *composition* by player count and shuffles their *order* with a seeded PRNG. The seed is a random value generated per squad per squad-local day and held in server memory (`lib/games/groupSeed.ts`, `GET /api/squads/[squadId]/games/group-seed`), so recreating a game the same day can't re-roll the grouping; an app restart issues a new seed.
